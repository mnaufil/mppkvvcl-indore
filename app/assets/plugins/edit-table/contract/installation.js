...
class BSTable2 {
    ...
    constructor(tableId, options) {
        ...
        var defaults = {
            ...
            advanced: { // Do not override advanced unless you know what youre doing
                ...
                <button type="button" class="btn btn-sm bEdit_Installation">
                    <span class="fe fe-edit fa-lg action-btn-table" > </span>
                </button>
                <button type="button" class="btn btn-sm bDel_Installation">
                    <span class="fe fe-trash-2 fa-lg action-btn-table" > </span>
                </button>
                <button  type="button" class="btn btn-sm bAcep_Installation" style="display:none;">
                    <span class="fe fe-check-circle fa-lg action-btn-table" > </span>
                </button>
                <button  type="button" class="btn btn-sm bCanc_Installation" style="display:none;">
        ...
        /** @private */
        this.actionsColumnHTML = '<td name="bstable-actions-installation">' + this.options.advanced.buttonHTML + '</td>';
     ...
     */
    init() {
        this.table.find('thead tr').prepend('<th name="bstable-actions-installation">' + this.options.advanced.columnLabel + '</th>'); // Append column to header
     ...
     */
    destroy() {
        this.table.find('th[name="bstable-actions-installation"]').remove(); //remove header
        this.table.find('td[name="bstable-actions-installation"]').remove(); //remove body rows
    }
    ...
    _actionsModeNormal(button) {
        $(button).parent().find('.bAcep_Installation').hide();
        $(button).parent().find('.bCanc_Installation').hide();
        $(button).parent().find('.bEdit_Installation').show();
        $(button).parent().find('.bDel_Installation').show();
        let $currentRow = $(button).parents('tr'); // get the row
        $currentRow.attr('data-status', ''); // remove editing status
    }
    _actionsModeEdit(button) {
        $(button).parent().find('.bAcep_Installation').show();
        $(button).parent().find('.bCanc_Installation').show();
        $(button).parent().find('.bEdit_Installation').hide();
        $(button).parent().find('.bDel_Installation').hide();
        let $currentRow = $(button).parents('tr'); // get the row
   ...
   _rowEdit(button) {
        ...
        data.action = 'editmaterial';
        data.rowId = rowIndex
        this.postdata(data);
        $("#table2-new-row-button-installation").hide();
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            ...
            if(i=='1')
            {
                var inputElement = "'dynamicequipmaterial'";
             input = '<input class="form-control input-sm"   data-original-value="' + content + '" id="dynamicequipmaterial'+rowIndex+'" value="' + content + '" onblur="charlimitwithrow(this.value, '+inputElement+', '+rowIndex+', 200)">';
              $td.html(div + input)
            }
            else if(i=='2')
            {
                
                 $.ajax({url: baseUrl+"loadunits/"+rowIndex, success: function(result){
                        inputs = result;
                        $td.html(div + result); 
                    }});
             ...
             {
                var inputElement = "'dynamictotalqtymaterial'";
             input = '<input type="number" class="form-control input-sm"   data-original-value="' + content + '" id="dynamictotalqtymaterial'+rowIndex+'" value="' + content + '" onblur="charlimitwithrow(this.value, '+inputElement+', '+rowIndex+', 7)">';
              $td.html(div + input)
             } 
    ...
    _rowDelete(button) {
        ...
        var data = {};
        data.sessionName = 'addcontract';
        data.action = 'deletematerial';
        data.rowId = rowIndex
         this.postdata(data);
         $("#table2-new-row-button-installation").show();
        $currentRow.remove();
        this.options.onDelete();

         var installationhiddentable = $("#installation_table").html();
        //console.log(milestonehiddentable);
        $("#installationhiddentable").val(installationhiddentable);
    ...
    _rowAccept(button) {
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            ...
            if(i==0)
               ...
               {
                    data.equipment_material_name  = cont;
               }
               else if(i==2)
               {
                    cont = $td.find('option:selected').text();
                    data.unit_id  = $td.find('select').val();
                    data.unit_text  = $td.find('option:selected').text();
               }
               else if(i==3)
               {
                    data.total_quantity  = cont;
               }
    ...
    _actionAddRow() {
        ...
        if ($allRows.length == 0) { // there are no rows. we must create them
            ...
            $cols.each(function(e) {
                ...
                if ($(column).attr('name') == 'bstable-actions-installation') {
    ...
    _modifyEachColumn($editableColumns, $cols, howToModify) {
        ...
        $cols.each(function() {
            n++;
            if ($(this).attr('name') == 'bstable-actions-installation') return; // exclude the actions column
    ...
    _addOnClickEventsToActions() {
        ...
        this.table.find('tbody tr .bEdit_Installation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowEdit(button) }
        });
        this.table.find('tbody tr .bDel_Installation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowDelete(button) }
        });
        this.table.find('tbody tr .bAcep_Installation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowAccept(button) }
        });
        this.table.find('tbody tr .bCanc_Installation').each(function() {
            let button = this;
    ...
    convertTableToCSV(separator) {
        ...
        _this.table.find('tbody tr').each(function() {
            ...
            if (_this.currentlyEditingRow($(this))) {
                $(this).find('.bAcep_Installation').click(); // Force Accept Edit
            }
            let $cols = $(this).find('td'); // read columns
            $currentRowValues = '';
            $cols.each(function() {
                if ($(this).attr('name') == 'bstable-actions-installation') {
