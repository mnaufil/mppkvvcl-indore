...
class BSTable4 {
    ...
    constructor(tableId, options) {
        ...
        if (tableId == 'new-milestone') {
            actionButtonHTML = `<div class="btn-list" >
                <button id="bEdit_Mobilisation" type="button" class="btn btn-sm" ${data}>
                    <span class="fe fe-edit fa-lg action-btn-table" > </span>
                </button>
                <button id="bAcep_Mobilisation" type="button" class="btn  btn-sm" ${data} style="display:none;">
                    <span class="fe fe-check-circle fa-lg action-btn-table" > </span>
                </button>
                <button id="bCanc_Mobilisation" type="button" class="btn  btn-sm" style="display:none;">
                    <span class="fe fe-x-circle fa-lg action-btn-table" > </span>
                </button>
            </div>`;
        } else {
            actionButtonHTML = `<div class="btn-list">
                <button type="button" class="btn btn-sm bEdit_Mobilisation">
                    <span class="fe fe-edit fa-lg action-btn-table" > </span>
                </button>
                <button type="button" class="btn  btn-sm bDel_Mobilisation">
                    <span class="fe fe-trash-2 fa-lg action-btn-table" > </span>
                </button>
                <button type="button" class="btn  btn-sm bAcep_Mobilisation" ${data} style="display:none;">
                    <span class="fe fe-check-circle fa-lg action-btn-table" > </span>
                </button>
                <button  type="button" class="btn  btn-sm bCanc_Mobilisation" style="display:none;">
                    <span class="fe fe-x-circle fa-lg action-btn-table" > </span>
                </button>
            </div>`
        }
    ...
    _actionsModeNormal(button) {
        $(button).parent().find('.bAcep_Mobilisation').hide();
        $(button).parent().find('.bCanc_Mobilisation').hide();
        $(button).parent().find('.bEdit_Mobilisation').show();
        $(button).parent().find('.bDel_Mobilisation').show();
        let $currentRow = $(button).parents('tr'); // get the row
        $currentRow.attr('data-status', ''); // remove editing status
    }
    _actionsModeEdit(button) {
        $(button).parent().find('.bAcep_Mobilisation').show();
        $(button).parent().find('.bCanc_Mobilisation').show();
        $(button).parent().find('.bEdit_Mobilisation').hide();
        $(button).parent().find('.bDel_Mobilisation').hide();
        let $currentRow = $(button).parents('tr'); // get the row
   ...
   _rowEdit(button) {
        ...
        data.action = 'editmobilisation';
        data.rowId = rowIndex
        this.postdata(data);
        
        let $cols = $currentRow.find('td'); // read rows
        $("#table2-new-row-button-mobilisation").hide();
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           // modify each column
            ...
            if(i=='0')
            {
				 ...
				 $.ajax({url: baseUrl+"loadmobilisationtype/"+rowIndex, success: function(result){
						inputs = result;
						$td.html(div + result); 
					}});
            ...
            {
                var inputElement = "'dynamicinvoicenummobilisation'";
             input = '<input class="form-control input-sm"   data-original-value="' + content + '" id="dynamicinvoicenummobilisation'+rowIndex+'" value="' + content + '" onblur="charlimitwithrow(this.value, '+inputElement+', '+rowIndex+', 50)">';
                $td.html(div + input)
            }
			else if(i=='2')
			{
				...
				var inputElement = '"dynamicinvoicedatemobilisation"';
//              input = '<input class="form-control" type="date" placeholder="dd-mm-yyyy" id="dynamicdatepickerstage'+rowIndex+'"  onblur="checkdateformat(this.value, "dynamicdatepickerstage",rowIndex)" >';
                ...
                input = "<input class='form-control' type='date' value='" + content + "' placeholder='dd-mm-yyyy'  id='dynamicinvoicedatemobilisation"+rowIndex+"' data-date='" + content + "' data-date-format='DD-MM-YYYY'  onblur='checkdateformat(this.value, "+inputElement+", "+rowIndex+")' >";
            ...
            else if(i=='4')
            {
                ...
                var inputElement = '"dynamicdatepaymentmobilisation"';
//              input = '<input class="form-control" type="date" placeholder="dd-mm-yyyy" id="dynamicdatepickerstage'+rowIndex+'"  onblur="checkdateformat(this.value, "dynamicdatepickerstage",rowIndex)" >';
                
                input = "<input class='form-control' type='date' value='" + content + "' placeholder='dd-mm-yyyy'  id='dynamicdatepaymentmobilisation"+rowIndex+"' data-date='" + content + "' data-date-format='DD-MM-YYYY'  onblur='checkdateformat(this.value, "+inputElement+", "+rowIndex+")' >";

                $td.html(div + input)
            }
            /*else if(i=='0')
            {
                input = '<input class="form-control" type="text" data-original-value="' + content + '" value="' + content + '"  >';
                $td.html(div + input)
            }*/
            
            else if(i=='3')
            {
                var inputElement = "'dynamicadvanceamountmobilisation'";
            input = '<input type="number" class="form-control input-sm" id="dynamicadvanceamountmobilisation'+rowIndex+'"   data-original-value="' + content + '" value="' + content + '" onblur="charlimitwithrow(this.value, '+inputElement+', '+rowIndex+', 12)">';
                $td.html(div + input)
            }
              else if(i=='5')
            {
                var inputElement = "'dynamicadvanceadjustedmobilisation'";
            input = '<input type="number" class="form-control input-sm" id="dynamicadvanceadjustedmobilisation'+rowIndex+'"   data-original-value="' + content + '" value="' + content + '" onblur="charlimitwithrow(this.value, '+inputElement+', '+rowIndex+', 12)">';
                $td.html(div + input)
            }
...
*/
    ...
    _rowDelete(button) {
        ...
        let $currentRow = $(button).parents('tr'); // access the row
         ...
         var data = {};
        data.sessionName = 'addcontract';
        data.action = 'deletemobilisation';
        data.rowId = rowIndex
         this.postdata(data);
        this.options.onBeforeDelete($currentRow);
        $("#table2-new-row-button-mobilisation").show();
        $currentRow.remove();
    ...
    _rowAccept(button) {
        ...
        //alert(rowIndex);
         var data = {};
        data.sessionName = 'addcontract';
        data.action = 'acceptmobilisation';
        data.rowId = rowIndex
            ...
            if(i==0)
        {
           ...
           /* //alert($td.find('option:selected').text());
             ...
             cont = $td.find('option:selected').text();
                    data.mobilisation_type  = $td.find('select').val();
                    data.mobilisation_text  = $td.find('option:selected').text();
              ...
              /* if(i==1)
               ...
               if(i==1)
               {
                    data.invoice_no  = cont;
               }
               else if(i==2)
               {
				   cont = $td.find('input').attr('data-date');
                    data.invoice_date  = cont;
               }
               else if(i==3)
               {
                    data.advance_amount  = cont;
               }
               else if(i==4)
               {
				   cont = $td.find('input').attr('data-date');
                    data.date_of_payment  = cont;
               }
               else if(i==5)
               {
                    data.advance_adjusted  = cont;
               }
         } 
    ...
    _actionAddRow() {
        ...
        if ($allRows.length == 0) { // there are no rows. we must create them
            ...
            $cols.each(function(e) {
                ...
                <button  type="button" class="btn btn-sm bAcep_Mobilisation"  style="display:none;">
                    <span class="fe fe-check-circle fa-lg action-btn-table" > </span>
                </button>
                <button type="button" class="btn btn-sm bCanc_Mobilisation" style="display:none;">
                    <span class="fe fe-x-circle fa-lg action-btn-table" > </span>
    ...
    _addOnClickEventsToActions() {
        ...
        this.table.find('tbody tr .bEdit_Mobilisation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowEdit(button) }
        });
        this.table.find('tbody tr .bDel_Mobilisation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowDelete(button) }
        });
        this.table.find('tbody tr .bAcep_Mobilisation').each(function() {
            let button = this;
            button.onclick = function() { _this._rowAccept(button) }
        });
        this.table.find('tbody tr .bCanc_Mobilisation').each(function() {
            let button = this;
