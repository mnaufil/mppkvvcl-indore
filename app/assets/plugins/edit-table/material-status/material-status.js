...
class BSTableMaterial {
    ...
    constructor(tableId, options) {
        ...
        var defaults = {
            ...
            advanced: { // Do not override advanced unless you know what youre doing
                ...
                <button id="bEdit" type="button" class="btn btn-sm b-Edit">
    ...
    _actionAddRow() {
        ...
        if ($allRows.length == 0) { // there are no rows. we must create them
            ...
            //Removing attribute
            $lastRow.find('#bEdit').removeAttr('data-material-status-details-id');
            $lastRow.find('#bDel').removeAttr('data-material-status-details-id');
    ...
    _addOnClickEventsToActions() {
        ...
        this.table.find('tbody tr #bEdit').each(function() {
            ...
            button.onclick = function() { showMaterialDetails(button) }
        });
