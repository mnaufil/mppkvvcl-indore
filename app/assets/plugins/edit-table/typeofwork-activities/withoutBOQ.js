...
class BSTableWithoutBOQ {
    ...
    constructor(tableId, options) {
        ...
        /*console.log('tableId: ' + this.table);
    ...
    _rowEdit(button) {
        ...
        $('#table2-new-row-button-withoutBOQ').prop('disabled', true);
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            ...
            if (i == 0) { /*Seq No*/
                ...
                input = '<input class="form-control input-sm type_of_activity" onkeyup="showWithoutBOQOptions(this)" data-original-value="' + content + '" value="' + content + '">';
                ...
                input = '<input class="form-control input-sm dashboard_head" onkeyup="showDashboardHeadOptions(this)" data-activity-type="withoutBOQ" data-original-value="' + content + '" value="' + content + '">';
                input += '<div id="dashboard-head" class="dropdown-view"></div>';
            } else if (i == 5) { /*Weightage*/                
                content = (content == '') ? '0' : content;
                input = '<input class="form-control input-sm"  data-original-value="' + content + '" value="' + content + '">';
            } else if (i == 6) { /*Report Head*/
                content = (content == '') ? '-' : content;
                input = '<input class="form-control input-sm report_head" onkeyup="showReportHeadOptions(this)" data-activity-type="withoutBOQ" data-original-value="' + content + '" value="' + content + '">';
    ...
    _rowDelete(button) {
        ...
        console.log($(hidden_input)); return false;
    ...
    _rowAccept(button) {
        ...
        /*let array_key = currentRow_id
        data_withoutBOQ.push(array_key);*/

        let content_arr = [];
        // console.log(data_withoutBOQ); return false;

        // Finish editing the row & save edits
        var i = 0;
        let row_accept = true;
        let seqno_cont, activity_group_cont, activity_name_cont, observations_cont, dashboard_head_cont, weightage_cont, report_head_cont, item_code_cont, erp_item_name_cont;
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            let cont;
            // let key;
            let toast_message;
            ...
            if (i == 0) { /*Seq No*/
                seqno_cont = cont = ($td.find('input').length == 1) ? $td.find('input').val() : $td.text(); // read through each input
                //key = 'seqno';
                // data_withoutBOQ[currentRow_id][key] = cont;
                // content_arr.push({'seqno': cont});
                // content_arr[key] = cont; 
                ...
                activity_group_cont = cont = ($td.find('input').length == 1) ? $td.find('input').val() : $td.text(); // read through each input
                //key = 'activity_group';
                // data_withoutBOQ[currentRow_id][key] = cont;
                ...
                activity_name_cont = cont = ($td.find('input').length == 1) ? $td.find('input').val() : $td.text(); // read through each input
                //key = 'activity_name';
                // data_withoutBOQ[currentRow_id][key] = cont;
                ...
                weightage_cont = cont = (seqno_cont == '' || activity_group_cont == '' || activity_name_cont == '') ? '' : $td.find('input').val(); // read through each input
                //key = 'weightage';
                // data_withoutBOQ[currentRow_id][key] = cont;
                // content_arr.push({'weightage': cont}); 
                // content_arr[key] = cont;
            } else if (i == 6) { /*Report Head*/
        ...
        if (row_accept == true) {
            ...
            content_arr.push({'seqno' : seqno_cont, 'activity_group' : activity_group_cont, 'activity_name' : activity_name_cont, 'observations' : observations_cont, 'dashboard_head' : dashboard_head_cont, 'weightage' : weightage_cont, 'report_head' : report_head_cont, 'item_code' : item_code_cont, 'erp_item_name' : erp_item_name_cont, 'activity_id' : activity_id});

            console.log('content_arr: ');
            console.log(content_arr);

            if (!$.isEmptyObject(data_withoutBOQ)) {
                $.each(data_withoutBOQ, function(index, value) {
                    $.each(value, function(ind, val) {
                        if (currentRow_id == ind) {
                            data_withoutBOQ.splice(index, 1);                            
                        }
                    });
                });

                data_withoutBOQ.push({[currentRow_id]: content_arr});
            } else {
                data_withoutBOQ.push({[currentRow_id]: content_arr});    
            }

            console.log(data_withoutBOQ);
            console.log(data_withoutBOQ.length);
        ...
        $('#table2-new-row-button-withoutBOQ').prop('disabled', false);
    }
    _rowCancel(button) {
        // Reject the changes
        let $currentRow = $(button).parents('tr'); // access the row
    ...
    _actionAddRow() {
        ...
        if ($allRows.length == 0) { // there are no rows. we must create them
            ...
            $cols.each(function(i) {
                ...
                {
                    console.log('correct');
                    $(column).html('<select class="form-control"><option>Select Activity Type</option><option selected>Civil Work</option><option>Sub station items<option></select>')
                }
                else if (i == '4')
                {
